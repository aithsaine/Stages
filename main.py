...
lb_title = tk.Label(app,text="Gestion De Stage",font="5",bd=10,bg='#c5efff')
lb_title.grid(column=0,row=0,columnspan=5,rowspan=3,pady=10,padx=120)
# id :
lb_id = tk.Label(app,text="Id",font=10,bg='#c5efff')
lb_id.grid(column=0,row=4,padx=10,sticky=tk.W)
enId = tk.Entry(app,font=15,bd=2)
enId.grid(row=4,column=1,padx=20)
#  nom stage : 
lb_name = tk.Label(app,text="Nom de stage",font=10,bg='#c5efff')
lb_name.grid(column=0,row=5,padx=10,sticky=tk.W)
enname = tk.Entry(app,font=15,bd=2)
enname.grid(row=5,column=1)
#  duree : 
lb_Duration = tk.Label(app,text="Duree",font=10,bg='#c5efff')
lb_Duration.grid(column=0,row=6,padx=10,sticky=tk.W)
enDuration = tk.Entry(app,font=10,bd=2)
enDuration.grid(row=6,column=1)

# prix
lb_price = tk.Label(app,text="Prix",font=10,bg='#c5efff')
lb_price.grid(column=0,row=7,padx=10,sticky=tk.W)
enPrice = tk.Entry(app,font=10,bd=2)
enPrice.grid(row=7,column=1)
# domaine
lb_Domaine = tk.Label(app,text="Domaine",font=10,bg='#c5efff')
lb_Domaine.grid(column=0,row=8,padx=10,sticky=tk.W)
enDomaine = tk.Entry(app,font=10,bd=2)
...
def Supprimer():
    app.destroy()
    os.startfile("deletepage.py")

    


btnInsert = tk.Button(app,text="Inserer",bg="#c1eeff",width=10,command=Inserer).grid(row=4, column=2,pady=5)
btnDelete = tk.Button(app,text="Supprimer",bg="#c1eeff",width=10,command=Supprimer)
btnDelete.grid(row=5, column=2,pady=5)
btnSearsh = tk.Button(app,text="Chercher",bg="#c1eeff",width=10).grid(row=6, column=2,pady=5)
btnUpdate = tk.Button(app,text="Modifier",bg="#c1eeff",width=10).grid(row=7, column=2,pady=5)
dd = tk.Checkbutton()
...
for item in stages:
    liste_des_tupple_stage.append((item['id'],item['nom'],item['duree'],"{:.2f}".format(item['prix']),item['domaine']))
